...
const riskCards = [
  {
    ...
    icon: (
      <svg width={22} height={22} viewBox="0 0 34 34" fill="none">
        <circle cx="17" cy="17" r="13" stroke="rgb(var(--c-brand-red))" strokeWidth={2.4} />
        <path d="M17 9v9" stroke="rgb(var(--c-brand-red))" strokeWidth={2.4} strokeLinecap="round" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width={22} height={22} viewBox="0 0 34 34" fill="none">
        <circle cx="12" cy="17" r="7" stroke="rgb(var(--c-ink))" strokeWidth={2.2} />
        <circle cx="22" cy="17" r="7" stroke="rgb(var(--c-gold))" strokeWidth={2.2} />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width={22} height={22} viewBox="0 0 34 34" fill="none">
        <rect x="5" y="18" width="5" height="10" rx="1.5" fill="rgb(var(--c-slate))" />
        <rect x="14.5" y="10" width="5" height="18" rx="1.5" fill="rgb(var(--c-ink))" />
        <rect x="24" y="14" width="5" height="14" rx="1.5" fill="rgb(var(--c-gold))" />
      </svg>
    ),
  },
];
  ...
  return (
    <main>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {riskCards.map((card) => (
              ...
              >
                ...
                <h3 className="mb-2.5 text-[17px] font-semibold text-ink">{card.title}</h3>
                <p className="mb-4 text-[14px] leading-relaxed text-slate">{card.desc}</p>
                <span className="inline-block rounded-full bg-tint px-2.5 py-1 font-mono text-[10.5px] tracking-wide text-ink">
                  {card.tag}
                </span>
              </div>
            ))}
          ...
          <div className="overflow-hidden rounded-xl2 border border-line bg-panel shadow-panel">
            <div className="grid md:grid-cols-2">
              <div className="p-8">
                ...
                <h4 className="mb-2 text-[16px] font-semibold text-ink">Historical simulation</h4>
              ...
              <div className="border-t border-line p-8 md:border-l md:border-t-0">
                ...
                <h4 className="mb-2 text-[16px] font-semibold text-ink">Currently running, in real time</h4>
                <p className="mb-4 text-[13.5px] leading-relaxed text-slate">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {roles.map((role) => (
              ...
              >
                ...
                <h3 className="mb-3 text-[16px] font-semibold text-ink">{role.title}</h3>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-4 border-t border-line px-6 py-8 text-[12.5px] text-slate md:px-10">
        <div className="flex items-center gap-2 font-mono text-[11.5px] tracking-wide text-ink">
